...
- 放置後選擇朝向後幹員正式部署並開始自動攻擊
- 放置後右邊會出現幹員的屬性、HP和類型

**3. DP 系統**

初始 10 DP，每秒 +1，每位幹員有不同部署費用；Cheat 模式下可無視限制部署任意幹員。
...
- 按畫面右上方的 `settings(齒輪)` 按鈕，可以切換 `Cheat Mode` 與 `Normal Mode`，以及中途放棄遊戲
  - `Cheat Mode` 切換可以不計點數部署，即使敵人衝進塔內，防守血量也不會扣
...
#### 郭東旭
這次的 OOPL 專案對我而言是一段難得的開發體驗。從一開始加入這個組別，我就清楚感受到，兩個人合作寫一個有規模的遊戲，難度遠比我預期的高——不只是技術層面，更多是在溝通與架構設計上的磨合。
專案初期，因為組員陳子揚已經搭好了基礎玩法的骨架，我的第一個任務不是新增功能，而是把現有的程式結構整理清楚。我們很快意識到，如果繼續在同一份程式碼上各自堆砌，兩人遲早會在合併時撞得頭破血流。於是我開始主導架構重構的部分，把場景系統梳理成 SceneManager 統一管理的堆疊，把關卡資料從遊戲邏輯中抽離出來，變成純資料驅動的 StageRepository。這個決策讓後來新增第二、三關的時候幾乎不需要動任何遊戲邏輯，只需要填資料，省了非常多時間。

到了中期，我向外擴充場景、UI 和細節。印象最深的是 SkewSprite 的實作：原作的大廳介面到處都是斜切的文字和平行四邊形按鈕，用一般的矩形 Sprite 根本做不出來，最後只能繞過框架直接操作 OpenGL 的 VBO，手動指定四個頂點座標，硬是把東西做出來的感覺，有一種說不上來的踏實。
當然整個開發過程並不總是順利的。敵人的攻擊動畫和傷害判定不同步、Yokai 的巡邏路點邏輯、程式結束時偶發的 segfault，這些問題有些解決了，有些只能留著遺憾。結束時的 segfault 到最後都沒有完全修好，有點可惜，但也算是提醒了我資源釋放的問題遠比想像中難處理。

期末繳交前，我負責拍了遊戲 demo 影片並剪輯上字幕，也把報告重新整理了一遍。整個流程回頭看，從架構設計到 UI 細節到最後的包裝，算是都有參與到。
總體而言，這次的開發讓我真正體會到，程式設計的難點不只是把功能做出來，而是讓程式碼的結構保持可讀、可擴充、可協作。這一點是我在課堂上很難學到的，但在這次真實的協作開發中，它變成了一種直覺。
