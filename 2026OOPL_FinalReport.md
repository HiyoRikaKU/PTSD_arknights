...
到了中期，兩人的職責已經相對清晰——陳子揚寫基礎核心玩法，我則往外擴充場景、UI 和細節。印象最深的是 SkewSprite 的實作：原作的大廳介面到處都是斜切的文字和平行四邊形按鈕，用一般的矩形 Sprite 根本做不出來，最後只能繞過框架直接操作 OpenGL 的 VBO，手動指定四個頂點座標，硬是把東西做出來的感覺，有一種說不上來的踏實。
